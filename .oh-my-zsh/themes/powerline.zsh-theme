...
ZSH_THEME_VIRTUAL_ENV_PROMPT_PREFIX="prefijo"
ZSH_THEME_VIRTUAL_ENV_PROMPT_SUFFIX="sufijo"
...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" $FG[242]"$'$(status_col)'$'\ue0b0'$'$(status_col)'"%F{black}"$'$(status_col)'$'$(git_prompt_info)$(git_prompt_status)$FG[065] '
        else
...
if [ $(id -u) -eq 0 ]; then
    POWERLINE_SEC1_BG=$FG[009]" root ✭"
    POWERLINE_SEC1_FG=$FG[235]
else
    POWERLINE_SEC1_BG=$BG[235]$FG[149]""
    POWERLINE_SEC1_FG=$FG[235]
fi
...
PROMPT="$POWERLINE_SEC1_BG$POWERLINE_SEC1_TXT$POWERLINE_USER_NAME%k%f$POWERLINE_SEC1_FG$BG[242]""%k%f%F{white}$BG[242] "$POWERLINE_CURRENT_PATH"%F{blue}"$POWERLINE_GIT_INFO_LEFT"%k"$'$(status_col2)'$'⮀'"%f "
